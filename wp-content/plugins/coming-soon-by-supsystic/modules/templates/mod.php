...
class templatesScs extends moduleScs {
	...
	public function loadBootstrapSimple() {
		static $loaded = false;
		if(!$loaded) {
			frameScs::_()->addStyle('bootstrap-simple', SCS_CSS_PATH. 'bootstrap-simple.css');
			$loaded = true;
		}
	}
	public function loadGoogleFont( $font ) {
		...
		if(!isset($loaded[ $font ])) {
			frameScs::_()->addStyle('google.font.'. str_replace(array(' '), '-', $font), 'https://fonts.googleapis.com/css?family='. urlencode($font));
			$loaded[ $font ] = 1;
		}
	}
}
