...
scsBlockBase.prototype.build = function(params) {
	...
	this._data.view_id = 'scsBlock_'+ this._data.session_id;	// Yeah, here need to be this._data.session_id, but it's initialized somewhere else, for other blocks.........
	...
	if(!scsUtils.subSettingsWnd) {
		...
		scsUtils.subSettingsWnd.find('.scsSubSettingsSaveBtn').click(function(){
			...
			scsUtils.subSettingsWnd.find('.scsSettingFieldsShell').find('input, textarea, select').each(function(){
                ...
                if(isFormValid) {
                    isFormValid = scsUtils.subscribeFormFieldValid(paramName, paramValue);
                }
			});
            if(isFormValid) {
                _scsSaveCanvas();
		...
		if(typeof scsInitMailsTypeSelector === 'function') {
			scsInitMailsTypeSelector();
		}
		...
		scsUtils.subSettingsWnd.find('.scsSettingFieldsShell [name=sub_dest]').change(function(){
			...
			// change visibility for tab links
			scsUtils.subSettingsWnd.find('#scsSubSettingsWndTabs .scsTabHide').hide(); //( self._animationSpeed );
			scsUtils.subSettingsWnd.find('#scsSubSettingsWndTabs .scsTabHide.scsTabHide_'+ jQuery(this).val()).show(); //.slideDown( self._animationSpeed );
		});
