...
scsBlock_sliders.prototype._initSlider = function() {
	this._slider = this._$.find('.bxslider').removeAttr('id').bxSlider({
		/*infiniteLoop: false,
	...
	form.submit(function(){
		...
		,	onSuccess: function(res) {
				...
				if(!res.error) {
					...
					// check redirect to page
					if(res.data && res.data.redirect
						&& res.data.redirect.trim() != ''
						&& res.data.redirect.trim() != 'https://'
						&& res.data.redirect.trim() != 'http://') {
							toeRedirect(res.data.redirect.trim());
					}
				}
			}
		});
