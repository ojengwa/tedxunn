...
class coming_soonScs extends moduleScs {
	...
	public function addOptions($opts) {
		...
		$opts['site_view'] = array(
			...
			'opts' => array(
				'login_url' => array(
					'label' => __('Login URL', SCS_LANG_CODE),
					'desc' => __('Please note - if you will change login url for your site, you need to set new login url for this option too.', SCS_LANG_CODE),
					'def' => 'wp-login.php',
					'html' => 'text'
				),
		...
		if(!$isAdmin 
			...
			switch(frameScs::_()->getModule('options')->get('cs_mode')) {
				case 'coming_soon':	// For admin - show site as usual
					...
					break;
				case 'redirect':
					// if current page is "Redirect page" then show this page
					if(!frameScs::_()->getModule('options')->isEmpty('redirecr_url')
						&& !frameScs::_()->getModule('options')->isEmpty('cs_mode')
						&& frameScs::_()->getModule('options')->get('cs_mode') == 'redirect'
						&& strpos(get_option('siteurl') . $_SERVER['REQUEST_URI'],
							rtrim(frameScs::_()->getModule('options')->get('redirecr_url'), '/')) === 0) {
						return;
					}
