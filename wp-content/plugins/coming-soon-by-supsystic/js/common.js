...
function scsInitMailsTypeSelector() {
	jQuery("#scs-mails-type-sel").on('change', function() {
		// reset prev Message
		var lblErrMsg = jQuery("#scs-subscr-err-msg");
		scsUtils.setErrorToLbl(lblErrMsg, '', false, 'scs-displ-none');
		if(jQuery(this).val() == 'newsletters') {
			if(jQuery("#scs-subscr-type-sel").val() == null) {
				jQuery("#scs-subscr-type-sel  :nth-child(1)").prop('selected', 'selected');
			}
            // check for Newsletter plugin installed
            scsUtils.subscribeFormFieldValid('sub_dest', 'newsletters');
		}
	});
}
